...
```{r plot-losses custom generator}
# Plot training losses (requires track_loss = TRUE during training)
if (!is.null(trained_gan$losses)) {
...
```{r visual-comparison custom generator, fig.width=10, fig.height=8}
# Compare distributions of continuous variables
par(mfrow = c(2, 3))
...
```{r categorical-comparison custom generator, fig.width=12, fig.height=6}
par(mfrow = c(2, 4))
...
For very deep networks, progressive training gradually increases the network capacity,
which can lead to more stable training and better results. We start with fewer layers
and progressively activate more.

```{r progressive-training}
# Create a deep generator with 5 layers
...
# Define epochs per stage (more epochs as we add capacity)
epochs_per_stage <- c(50, 75, 100, 125, 150)

# Progressive training loop
cat("=== Progressive Training ===\n")

for (stage in 1:5) {
  # Set number of active blocks
 deep_gen$set_active_blocks(stage)
  cat(sprintf("\nStage %d: Training with %d active blocks for %d epochs\n",
              stage, stage, epochs_per_stage[stage]))

  # Train for this stage
  # Note: We pass the generator directly to continue training the same model
  stage_result <- gan_trainer(
    data = train_data,
    noise_dim = 128,
    generator = deep_gen,
    value_function = "wgan-gp",
    ...
    base_lr = 1e-4 * (0.9 ^ (stage - 1)),  # Slightly reduce LR each stage
    ttur_factor = 2,
    pac = 10,
    lr_schedule = "cosine",
    track_loss = TRUE,
  ...
  # Update the generator reference (it's been trained)
  deep_gen <- stage_result$generator

  cat(sprintf("Stage %d complete. Generator has %d parameters.\n",
              stage, sum(sapply(deep_gen$parameters, function(p) p$numel()))))
}

# Sample from the progressively trained generator
synthetic_progressive <- sample_synthetic_data(stage_result, transformer, n = 1000)

cat("\n=== Progressive Training Complete ===\n")
cat("Total epochs:", sum(epochs_per_stage), "\n")
```

```{r progressive-comparison, fig.width=10, fig.height=4}
# Quick comparison of progressive training results
par(mfrow = c(1, 3))

# Age distribution
hist(adult$age, col = rgb(0.2, 0.4, 0.6, 0.5), main = "Age",
     xlab = "Age", breaks = 30, freq = FALSE)
hist(synthetic_progressive$age, col = rgb(0.8, 0.2, 0.2, 0.5),
     add = TRUE, breaks = 30, freq = FALSE)
legend("topright", c("Real", "Synthetic"),
       fill = c(rgb(0.2, 0.4, 0.6, 0.5), rgb(0.8, 0.2, 0.2, 0.5)))

# Hours per week
hist(adult$hours_per_week, col = rgb(0.2, 0.4, 0.6, 0.5),
     main = "Hours per Week", xlab = "Hours", breaks = 30, freq = FALSE)
hist(synthetic_progressive$hours_per_week, col = rgb(0.8, 0.2, 0.2, 0.5),
     add = TRUE, breaks = 30, freq = FALSE)

# Income distribution
real_income <- prop.table(table(adult$income))
synth_income <- prop.table(table(synthetic_progressive$income))
barplot(rbind(real_income, synth_income), beside = TRUE,
        col = c(rgb(0.2, 0.4, 0.6, 0.7), rgb(0.8, 0.2, 0.2, 0.7)),
        main = "Income", legend = c("Real", "Synthetic"))
```
