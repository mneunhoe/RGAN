...
By default, `dp_gan_trainer()` uses **cryptographically secure** random number generation
via OpenDP (`secure_rng = TRUE`). This is required for formal privacy guarantees but is
slower. For development and testing, you can set `secure_rng = FALSE` to use torch's
standard (faster) RNG:
- **Production/publishing**: Use `secure_rng = TRUE` (default) for valid privacy guarantees
- **Development/testing**: Use `secure_rng = FALSE` for faster iteration
...
# Check for OpenDP availability (only required when secure_rng = TRUE)
has_opendp <- requireNamespace("opendp", quietly = TRUE)
if (!has_opendp) {
  message("OpenDP not available. Examples will use secure_rng = FALSE (faster but not cryptographically secure)")
}
...
dp_gan <- dp_gan_trainer(
  ...
  # === Security ===
  # Use secure_rng = TRUE (default) for production with valid privacy guarantees

  # Use secure_rng = FALSE for faster development/testing
  secure_rng = has_opendp
)
...
for (eps in epsilon_values) {
  ...
  dp_models[[as.character(eps)]] <- dp_gan_trainer(
    ...
    device = device,
    seed = 42,
    secure_rng = has_opendp  # Use secure RNG if available
  )
...
For **end-to-end privacy guarantees**, both the GAN training and the post-GAN boosting
must be differentially private. This is achieved by:

1. Training with `dp_gan_trainer()` (DP-SGD for the discriminator)
2. Applying `apply_post_gan_boosting()` with `dp = TRUE` (exponential mechanism)

The total privacy budget is the **composition** of both steps. For example, if training
uses ε₁ = 5 and boosting uses ε₂ = 1, the total privacy cost is approximately ε₁ + ε₂ = 6
(under basic composition; tighter bounds are possible with advanced composition).

**Important**: The discriminator must output scores bounded between 0 and 1 (sigmoid
activation) for the privacy analysis of DP post-GAN boosting to be valid. The
`dp_gan_trainer` uses sigmoid by default.

### Training a DP-GAN with Checkpoints

Use `dp_gan_trainer` with `checkpoint_epochs` to enable post-GAN boosting:

```{r dp-pgb-training}
# Train with DP-SGD and save checkpoints for post-GAN boosting
# The discriminator uses sigmoid output by default (required for valid privacy analysis)
dp_pgb_gan <- dp_gan_trainer(
  data = transformed_data,

  # === Privacy Parameters for Training ===
  target_epsilon = 5.0,     # Privacy budget for DP-SGD training
  ...
  checkpoint_epochs = 5,    # Save every 5 epochs
...
cat("Training epsilon:", dp_pgb_gan$privacy$final_epsilon, "\n")
cat("Checkpoints captured at epochs:", dp_pgb_gan$checkpoints$epochs, "\n")
```

### Applying DP Post-GAN Boosting

Now apply post-GAN boosting with differential privacy. The `dp = TRUE` flag uses
the exponential mechanism for private discriminator selection:
...
boosted_result <- apply_post_gan_boosting(
  ...
  n_candidates = 1000,
  steps = 200,
  dp = TRUE,              # Enable differential privacy for boosting
  MW_epsilon = 1.0,       # Privacy budget for multiplicative weights
  seed = 123
)

# Calculate total privacy budget (basic composition)
training_epsilon <- dp_pgb_gan$privacy$final_epsilon
boosting_epsilon <- 1.0  # MW_epsilon
total_epsilon <- training_epsilon + boosting_epsilon

cat("Training epsilon:", training_epsilon, "\n")
cat("Boosting epsilon:", boosting_epsilon, "\n")
cat("Total epsilon (basic composition):", total_epsilon, "\n")
cat("DP-Boosted samples:", boosted_result$n_unique, "\n")
```

### Comparing DP Training vs DP Training + Boosting

```{r compare-dp-pgb, fig.width=12, fig.height=4}
# Generate synthetic data from DP-trained GAN (no boosting)
dp_synthetic <- sample_synthetic_data(dp_pgb_gan, transformer, n_samples = 2000)
...
# DP-trained synthetic (no boosting)
plot(dp_synthetic, col = rgb(0.8, 0.4, 0.1, 0.5), pch = 19,
     main = sprintf("DP Training (ε = %.1f)", training_epsilon),
     xlab = "x", ylab = "y", asp = 1,
     xlim = c(-5, 5), ylim = c(-5, 5))

# DP-trained + DP-boosted synthetic
plot(boosted_synthetic[, 1:2], col = rgb(0.2, 0.7, 0.3, 0.5), pch = 19,
     main = sprintf("DP Training + Boosting (ε = %.1f)", total_epsilon),
     xlab = "x", ylab = "y", asp = 1,
     xlim = c(-5, 5), ylim = c(-5, 5))
```
...
- **secure_rng**: Use `secure_rng = FALSE` during hyperparameter tuning for faster
  iteration, then switch to `secure_rng = TRUE` for final training runs.
...
2. **Computational cost**: Per-sample gradient computation is slower than batch
   gradients. Consider using smaller models or fewer epochs. For faster iteration
   during development, use `secure_rng = FALSE`.
...
3. **Privacy accounting**: The privacy guarantees assume correct implementation
   ...
   cryptographically secure operations when `secure_rng = TRUE` (default).
...
5. **Secure vs fast RNG**: The `secure_rng` parameter controls whether to use
   cryptographically secure random number generation:
   - `secure_rng = TRUE` (default): Uses OpenDP for secure noise. **Required for
     valid privacy guarantees** when publishing results or deploying models.
   - `secure_rng = FALSE`: Uses torch's standard RNG. Significantly faster but
     **does not provide cryptographic security**. Useful for development/testing.
